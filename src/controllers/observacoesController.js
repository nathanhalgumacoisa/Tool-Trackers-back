...
export async function getObservacoesByParam (req, res) {
    ...
    try {
        let result;
        if (isNaN(param)) {
            result = await pool.query('SELECT * FROM observacoes WHERE categoria LIKE $1;', [`%${param}%`]);
        } else {
            result = await pool.query('SELECT * FROM observacoes WHERE categoria = $1;', [param]);
        }
...
export async function createObservacoes (req, res)  {
    try {
        ...
        const result = await pool.query(
            'INSERT INTO observacoes (nome_organizador, numero_organizador) VALUES ($1, $2) RETURNING *;',
